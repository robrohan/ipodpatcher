/*
 * FastUnicodeCompare has been publically provided by Apple, see Technote 1150
 */

#define SHORT_TABLES 1

#if SHORT_TABLES

    static uint16 gZeroValueTable[] = {
        // this lists values that get turned into 0 for comparison
        0xfeff,
...
static uint16 fuc_convert (uint16 c1) {
    if (c1 < 0x0100) {
        if (c1 == 0) {
            c1 = 0xffff; // one of the special cases
        } else {
            c1 = gLowerCaseTable[c1];
        }
    } else {
        // search the list of values that should be converted to 0
        for (uint16 *p = gZeroValueTable; *p; ++p) {
            if (*p == c1) {
                c1 = 0;
                break;
            }
...
static int32 FastUnicodeCompare ( const uint16be* str1, int length1,
                                  const uint16be* str2, int length2)
/*
 * Background of this function:
 *
 * On a HFS dir, all dir entries are organized in one big B-Tree, and
 * this tree uses the file names as part of the node IDs. This means
 * that when searching for a file in the tree, the order of the names
 * in it is very important. This comparison function makes sure that
 * the compare is using the same ordering rules so that the binary search
 * can work.
 * If we're remove this conversion, we might end up with an algorithm
 * that only works as long as there are no non-ASCII file names on the
 * entire disk. But non-english users might then have sporadic problems
 * of "file not found" errors that no one can explain. So, be VERY careful
 * before you decide to remove this code and the tables!
 *
 * Example: If a file name contains a 0-char (which is actually
 * possible on HFS, although not encouraged by Apple), then this 0 gets
 * converted to value 0xffff for comparison, so that any other char is
 * lower. So, if we would not do this conversion below and had a name with
 * a zero-char in the directory, then it _might_ happen that the search
 * for a name passes this 0-containing node. If the comparison gives the
 * wrong result because we do not make the 0 into a 0xffff for the compare,
 * the entire file name lookup would turn out wrong.
 ...
 * value as a "�", or something like that.
 */
{
    uint16     c1,c2;
        ...
        //
        // It simplifies the table lookup based on this assumption:
        //   One of the two strings is the name that's searched in the
        //   directory.
        //   That name is supposed to be using only 7bit ASCII chars
        //   The other item needs to be found lower or higher. If it
        //   contains any chars above ASCII, we do not have to special-
        //   handles those chars much any more as detailed in the big
        //   table. Instead, with a few exceptions, they are compared
        ...
        while (1) {
            ...
            if (c1 != c2)       //  found a difference, so stop looping
                break;
            if (0 == c1)        //  did we reach the end of both strings at the same time?
                return 0;       //  yes, so strings are equal
        }

   #else

        // This is the original and faster form using tables
